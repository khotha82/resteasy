...
function program11(depth0,data) {
  ...
  return "\n          <div class='sandbox_header'>\n            <div class='submit button' name='commit'>Try it out !</div>\n            <div class='dummySubmit button' name='dummySubmit'>Sample Response</div>\n            <a href='#' class='response_hider' style='display:none'>Hide Response</a>\n            <img alt='Throbber' class='response_throbber' src='images/throbber.gif' style='display:none' />\n          </div>\n          ";
  }
...
(function() {
  ...
  OperationView = (function(_super) {
    ...
    OperationView.prototype.events = {
      ...
      'click .dummySubmit': 'submitOperation',
    ...
    OperationView.prototype.submitOperation = function(e) {
      ...
      error_free = true;
      //Blocking the below part to show the Dummy portion W/O any validation
      if(!$(e.currentTarget).hasClass('dummySubmit')){
      ...
      if (error_free) {
		...
		//If the user is clicked on the dummySubmit, add a dummy variable in the map to stop the service call
		if(!$(e.currentTarget).hasClass('dummySubmit')){
			map["dummy"] = false;
		}
		else{
    ...
    OperationView.prototype.handleFileUpload = function(map, form) {
      ...
      alert ('headerParams::'+headerParams);
      obj = {
    ...
    OperationView.prototype.showResponse = function(response) {
      ...
      return $(".response_body", $(this.el)).html(escape(prettyJson));
       ...
       OperationView.prototype.showErrorStatus = function(data, parent) {
    	var resourceName =  parent.model.resourceName;
    	var nickname= parent.model.nickname;
        var method = parent.model.method;
        var divId_response = '#'+resourceName+"_"+nickname+'_response';
    	var divId_request = '#'+resourceName+"_"+nickname+'_request';
    	
    	if(method == 'get'){
		    var jsondata =$('iframe').contents().find(divId_response).html();
			return parent.showStatus(data, jsondata);
    ...
    OperationView.prototype.showStatus = function(data,jsn) {
      var code, content, contentType, headers, pre, response_body,request_body;
      ...
      } else if (contentType != undefined && ( contentType.indexOf("application/json") === 0 || contentType.indexOf("application/hal+json") === 0)) {
        code = $('<code />').text(JSON.stringify(JSON.parse(content), null, 2));
        pre = $('<pre class="json" />').append(code);
      } else if (contentType != undefined && contentType.indexOf("application/xml") === 0) {
        code = $('<code />').text(this.formatXml(content));
        pre = $('<pre class="xml" />').append(code);
      } else if (contentType != undefined && contentType.indexOf("text/html") === 0) {

        code = $('<code />').html(/*"<pre>" + JSON.stringify(jsn, null, " ").replace(/\n/g, "<br>") + "</pre>"*/jsn);
        pre = $('<pre class="xml" />').append(code);
      } else if (contentType != undefined && contentType.indexOf("image/") === 0) {
      ...
      response_body = pre;
      //request_body = pre;
      $(".request_url", $(this.el)).html("<pre>" + window.location.host + data.request.path + "</pre>");
      $(".response_code", $(this.el)).html("<pre>" + data.status + "</pre>");
      $(".response_body", $(this.el)).html(response_body);
      //$(".request_body", $(this.el)).append(pre);
      $(".response_headers", $(this.el)).html("<pre>" + JSON.stringify(data.getHeaders(), null, "  ").replace(/\n/g, "<br>") + "</pre>");
      $(".response", $(this.el)).slideDown();
      $(".response_hider", $(this.el)).show();
      $(".response_throbber", $(this.el)).hide();
      //if(parent.model.method=='post'){
    	  //$('.request_header',$(this.el)).show();
  		//$('.request_body').css('display','block');
      //}
