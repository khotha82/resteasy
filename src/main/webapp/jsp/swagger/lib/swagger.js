...
(function() {
  ...
  SwaggerOperation = (function() {
    ...
    SwaggerOperation.prototype.urlify = function(args) {
	  ...
	  var hostUrl, swaggerUrl,swaggerTargetUrl;
	  url = this.resource.basePath;
	  if (args["dummy"] != true){
		url = url.substring(0,this.resource.basePath.lastIndexOf('openapi')+'openapi'.length)
    ...
    if ((queryParams != null) && queryParams.length > 0) {
      ...
      hostUrl = window.location.origin + url;
	  if($("#swaggerHostUrl").html() != null  && $("#swaggerHostUrl").html() != undefined && $("#swaggerHostUrl").html() != ""){
    	swaggerUrl = window.location.protocol + "//" + $("#swaggerHostUrl").html() + url;
	  }
	  else{
		swaggerUrl = hostUrl;
	  }
	  
	  if($("#swaggerTargetHostUrl").html() != null  && $("#swaggerTargetHostUrl").html() != undefined && $("#swaggerTargetHostUrl").html() != ""){
		  	swaggerTargetUrl = window.location.protocol + "//" + $("#swaggerTargetHostUrl").html() + url;
		  }
		  else{
			swaggerTargetUrl = hostUrl;
		  }
      
	  //Code Chnages for the Authorization start
	  if($("#requestForm") != undefined && args["dummy"] != true){
		$("#requestForm input[name=URL]").val(swaggerTargetUrl);
	  }
	  else{
		  swaggerUrl = window.location.origin + '/dummy.json';
	  }
	  //Code Chnages for the Authorization END
	  return swaggerUrl;
    };
  ...
  SwaggerRequest = (function() {
    function SwaggerRequest(type, url, params, opts, successCallback, errorCallback, operation, execution) {
      ...
      freshNonce();
      freshTimestamp();
      $("#requestForm input[name=parameters]").val('');
      $("#requestForm input[name=oauth_consumer_key]").val($("#ock").html());
      $("#requestForm input[name=consumerSecret]").val($("#osk").html());
      $("#requestForm input[name=httpMethod]").val(this.type);
      sign(document.request);
      window.authorizations.add("key", new ApiKeyAuthorization("Authorization", $("#authorizationHeader").html() , "header"));
      //Code Chnages for the Authorization end
